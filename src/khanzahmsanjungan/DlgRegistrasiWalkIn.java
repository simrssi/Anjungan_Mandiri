...
public class DlgRegistrasiWalkIn extends javax.swing.JDialog {
    ...
    private String umur = "0", sttsumur = "Th", hari = "", kode_dokter = "", kode_poli = "",pen_jab="",kd_penjab="", nama_instansi, alamat_instansi, kabupaten, propinsi, kontak, email;
...
//    Private String x =  cmbCaraBayar.getSelectedItem().toString();
    ...
    public DlgRegistrasiWalkIn(java.awt.Frame parent, boolean id) {
        ...
        Penjab.addWindowListener(new WindowListener() {
            ...
            public void windowClosed(WindowEvent e) {
                kd_penjab = "";
                CaraBayar.setText(Penjab.getTable().getValueAt(Penjab.getTable().getSelectedRow(), 1).toString());
                kd_penjab = Penjab.getTable().getValueAt(Penjab.getTable().getSelectedRow(), 0).toString();
    ...
    private void initComponents() {
        ...
        btnSimpan3.setForeground(new java.awt.Color(0, 131, 62));
        ...
        btnSimpan3.setFont(new java.awt.Font("Poppins", 0, 18)); // NOI18N
        ...
        jPanel2.add(btnSimpan3);
        btnSimpan3.setBounds(810, 220, 70, 40);

        CaraBayar.setEditable(false);
        CaraBayar.setFont(new java.awt.Font("Poppins", 0, 18)); // NOI18N
        CaraBayar.setPreferredSize(new java.awt.Dimension(72, 28));
        ...
        CaraBayar.setBounds(290, 220, 520, 40);
...
//        } 
        else if (Sequel.cariInteger("select count(no_rkm_medis) from reg_periksa where kd_pj='"+kd_penjab+"' and no_rkm_medis='" + lblNoRM.getText() + "' and kd_poli='" + kode_poli + "' and kd_dokter='" + kode_dokter + "' and tgl_registrasi='" + Valid.SetTgl(TanggalPeriksa.getSelectedItem() + "") + "' ") > 0) {
            ...
            if (Sequel.menyimpantf2("reg_periksa", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "No.Rawat", 19,
                    new String[]{"W"+NoReg.getText(), NoRawat.getText(), Valid.SetTgl(TanggalPeriksa.getSelectedItem() + ""), Sequel.cariIsi("select current_time()"),
                        kode_dokter, lblNoRM.getText(), kode_poli, TPngJwb.getText(), TAlmt.getText(), THbngn.getText(), biayareg, "Belum",
                        "Lama", "Ralan",kd_penjab , umur, sttsumur, "Belum Bayar", status}) == true) {
...
//            Sequel.cariIsi("select penjab.png_jawab from penjab where penjab.kd_pj=?",CaraBayar,kdpnj.getText());
...
//        }
            if(evt.getKeyCode()==KeyEvent.VK_PAGE_DOWN){
               Sequel.cariIsi("select png_jawab from penjab where kd_pj=?", CaraBayar,kdpnj.getText());
           }else if(evt.getKeyCode()==KeyEvent.VK_ENTER){
               Sequel.cariIsi("select png_jawab from penjab where kd_pj=?", CaraBayar,kdpnj.getText());
//               BtnAll.requestFocus();
           }else if(evt.getKeyCode()==KeyEvent.VK_PAGE_UP){
...
//               BtnSeek3ActionPerformed(null);
    ...
    private void btnSimpan3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSimpan3ActionPerformed
        // TODO add your handling code here:
        Penjab.tampil();
        Penjab.setSize(jPanel1.getWidth() - 50, jPanel1.getHeight() - 50);
        Penjab.setLocationRelativeTo(jPanel2);
        Penjab.setVisible(true);
//        Penjab.isCek();
//        Penjab.setSize(jPanel1.getWidth()-20,jPanel1.getHeight()-20);
//        Penjab.setLocationRelativeTo(jPanel1);
